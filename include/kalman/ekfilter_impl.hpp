...
namespace Kalman {
  ...
  /* T   := float or double. Type of date in matrix elements.
   * BEG := 0 or 1. Starting index of vector.
   * OQ  := true if Q is always a diagonal matrix
   * OVR := true if Both V & R are diagonal matrices
   * DGB := debug
   */
  template<typename T, K_UINT_32 BEG, bool OQ, bool OVR, bool DBG>
  ...
  void EKFilter<T, BEG, OQ, OVR, DBG>::factor(Matrix& P_) {
    ...
    // Don't verify that P_ is a square matrix (i.e. P_.ncol() == P_.nrow())
    // since like a, even if the nrow() < ncol() we can still factor the
    // square submatrix o P and use that to factorize U.
